...
public class ChunkLoadDetector implements Detector {

    private final BobbyPlugin plugin;
    private final PluginRegistry registry;

    public ChunkLoadDetector(BobbyPlugin plugin, PluginRegistry registry) {
        this.plugin = plugin;
    ...
    public String getName() {
        return "Детектор генерации чанков";
    }

    @Override
    public CompletableFuture<Void> scan() {
        CompletableFuture<Void> future = new CompletableFuture<>();
        // Эмуляция - обычно должно отслеживать событие ChunkLoadEvent асинхронно
